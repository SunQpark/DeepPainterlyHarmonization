...
def loss_content(output, refer, mask):
    # # loss = torch.zeros((len(indices), ), device=device, dtype=torch.float32)
    # losses = []
    # # idx = 0
    # for i in indices:
    #     losses.append(F.mse_loss(
    #         select_mask(output[i].squeeze(0), mask[i]), 
    #         select_mask(refer[i][1], mask[i])).unsqueeze(0))
    #     # idx += 1
...
def loss_style(output, refer, mask):
    ...
    # for i in indices:
    #     losses.append(F.mse_loss(
    #         gram(select_mask(output[i].squeeze(0), mask[i])),
    #         gram(select_mask(refer[i][0], mask[i]))).unsqueeze(0))
    #     # idx += 1
...
def loss_style_wrapper(output, refer, mask):
    pass
...
def stage1_loss(fts_target, fts_refer, masks):
    ...
    for i, (target, refer, mask) in enumerate(zip(fts_target, fts_refer, masks)):
        ...
        # if i in s_layers:
        #     # loss += s_loss
        #     s_loss = loss_style(target.squeeze(0), ref_s, mask) * w_s
        #     # break
        # else:
...
if __name__ == '__main__':
    ...
    save_image(out_image, 'output.png')
